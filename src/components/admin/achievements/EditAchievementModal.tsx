...
import { Loader2, Image as ImageIcon, FileText, Save } from 'lucide-react';
...
interface EditAchievementModalProps {
  ...
  onClose: () => void;
  onSave: (id: string, updates: Partial<Achievement>) => Promise<Achievement>;
}
...
const EditAchievementModal: React.FC<EditAchievementModalProps> = ({
  ...
  useEffect(() => {
    if (achievement) {
      setFormData({
        ...
        file_type: achievement.file_type || 'image'
      });
    }
  }, [achievement]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!achievement || !formData.title.trim()) {
      toast({
        title: 'Error',
