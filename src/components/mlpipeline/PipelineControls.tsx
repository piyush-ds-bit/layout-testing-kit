...
const PipelineControls: React.FC<PipelineControlsProps> = ({
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-3 p-4 rounded-lg bg-[#182437]/90 border border-portfolio-accent/30 backdrop-blur-sm"
    >
      {/* Play/Pause Button */}
      <button
        onClick={isPlaying && !isPaused ? onPause : onPlay}
        className="p-2 rounded-md bg-portfolio-accent/20 hover:bg-portfolio-accent/30 transition-colors"
        aria-label={isPlaying && !isPaused ? "Pause" : "Play"}
      >
        {isPlaying && !isPaused ? (
          <Pause className="w-5 h-5 text-portfolio-accent" />
        ) : (
          <Play className="w-5 h-5 text-portfolio-accent" />
        )}
      </button>
            ...
            <button
              ...
              className={`px-2 py-1 rounded text-xs font-medium transition-colors ${
                speed === s
                  ...
                  : "bg-portfolio-accent/20 text-gray-300 hover:bg-portfolio-accent/30"
              }`}
            >
              {s}x
            </button>
