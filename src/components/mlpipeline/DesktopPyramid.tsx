  ...
  return (
    <div className="hidden md:flex flex-col items-center gap-0 w-full">
      ...
      <div className="flex justify-center mb-2 md:mb-6">
        <AnimatedArrowDown isActive={isArrowActive(2)} particleType="processed" />
      </div>
      {/* Row 2 - 3 steps */}
      <div className="flex flex-row justify-center items-start gap-4 mb-2 md:mb-6">
        {row2.map((step, idx) => (
          <React.Fragment key={step.label}>
            {renderNode(step, idx + 3)}
            {idx < row2.length - 1 && (
              <AnimatedArrowRight 
                isActive={isArrowActive(idx + 3)} 
                particleType={getParticleType(idx + 3)} 
              />
            )}
          </React.Fragment>
        ))}
      ...
      {/* Down arrow to row 3 */}
      <div className="flex justify-center mb-2 md:mb-6">
        <AnimatedArrowDown isActive={isArrowActive(5)} particleType="predictions" />
      </div>
      {/* Row 3 - 3 steps */}
      <div className="flex flex-row justify-center items-start gap-4 mb-2 md:mb-6">
        {row3.map((step, idx) => (
          <React.Fragment key={step.label}>
            {renderNode(step, idx + 6)}
            {idx < row3.length - 1 && (
              <AnimatedArrowRight 
                isActive={isArrowActive(idx + 6)} 
