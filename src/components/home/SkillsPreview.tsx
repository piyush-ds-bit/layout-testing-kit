  ...
  return <section id="skills" className="portfolio-section">
      ...
      boxShadow: '0 6px 32px 0 rgba(76,201,240,0.14), 0 2px 8px rgba(10,20,30,0.18), 0 1.5px 36px 0 rgba(0,0,0,0.13)'
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <Link to="/skills" className="flex items-center text-portfolio-accent hover:text-portfolio-accent-dark transition-colors">
            View All <ArrowRight className="ml-2 w-4 h-4" />
          </Link>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
          {previewCategories.map(category => <div key={category.id} className="portfolio-card-hover p-6 bg-[#1a202c] rounded-lg shadow-md">
              ...
              <div className="grid grid-cols-2 gap-3">
                {category.skills.slice(0, 4).map(skill => <div key={skill.id} className="flex items-center gap-2 py-2 px-4 rounded-full bg-[#1e2738] border border-[#2d3748]">
