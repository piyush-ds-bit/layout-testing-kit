...
const Navbar: React.FC = () => {
  ...
  return <>
      <header className="sticky top-0 z-50 w-full bg-portfolio-darkest/80 backdrop-blur-md border-b border-portfolio-dark">
          ...
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center space-x-4">
              <DropdownMenu>
                ...
                <DropdownMenuContent className="min-w-[200px] bg-portfolio-darkest/95 backdrop-blur-md border border-portfolio-accent/30 shadow-xl shadow-portfolio-accent/10" align="end" sideOffset={8}>
                  {exploreItems.map(item => <DropdownMenuItem key={item.path} className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-all duration-200 text-white hover:bg-portfolio-accent/20 hover:text-portfolio-accent focus:bg-portfolio-accent/20 focus:text-portfolio-accent ${isActiveItem(item) ? 'bg-portfolio-accent/10 text-portfolio-accent' : ''} ${item.label === 'Blogs' ? 'blog-glow-item' : ''}`} onClick={() => handleNavClick(item)}>
              ...
              {user ? <div className="flex items-center ml-2 space-x-3">
                ...
                </div> : <Link to="/login" className="ml-2">
                  <Button variant="outline" className="border-portfolio-accent text-portfolio-accent hover:bg-portfolio-accent hover:text-white">
                    Login
                  </Button>
                </Link>}
        ...
        <div className={`md:hidden fixed top-[62px] left-0 right-0 bg-portfolio-darkest/95 backdrop-blur-md border-b border-portfolio-dark transform ${isOpen ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"} transition-all duration-300 ease-in-out`}>
          <div className="p-4 space-y-2">
            {/* Explore Accordion */}
            <button onClick={() => setMobileExploreOpen(!mobileExploreOpen)} className="explore-button w-full flex items-center justify-between px-4 py-3 rounded-lg text-white font-medium transition-all duration-300">
            ...
            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${mobileExploreOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
              <div className="pl-4 space-y-1">
                {exploreItems.map(item => <button key={item.path} onClick={() => handleNavClick(item)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-white/80 hover:text-portfolio-accent hover:bg-portfolio-accent/10 transition-all duration-200 ${isActiveItem(item) ? 'bg-portfolio-accent/10 text-portfolio-accent' : ''} ${item.label === 'Blogs' ? 'blog-glow-item' : ''}`}>
                    <item.icon className="w-4 h-4" />
                    <span>{item.label}</span>
                  </button>)}
              </div>
            ...
            {/* Auth Section */}
            {user ? <div className="pt-4 border-t border-portfolio-dark space-y-2">
                <span className="block px-4 text-portfolio-gray-light text-sm">
                  Hi, {user.user_metadata?.name || user.email?.split('@')[0] || 'User'}
                </span>
                {isAuthorized && <Link to="/contact-messages" onClick={() => setIsOpen(false)}>
              ...
              </div> : <Link to="/login" onClick={() => setIsOpen(false)} className="block pt-4 border-t border-portfolio-dark">
                <Button variant="outline" className="w-full border-portfolio-accent text-portfolio-accent hover:bg-portfolio-accent hover:text-white">
                  Login
                </Button>
              </Link>}
      ...
      {showMobileNav && <nav className="mobile-bottom-nav md:hidden">
          {exploreItems.slice(0, 5).map(item => <button key={item.path} onClick={() => handleNavClick(item)} className={`mobile-bottom-nav-item ${isActiveItem(item) ? 'active' : ''}`}>
