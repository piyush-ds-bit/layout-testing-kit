...
const Navbar: React.FC = () => {
  ...
  return <>
        ...
        <div className="portfolio-container py-4">
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center space-x-2">
              {navItems.map(item => item.path === '/' ? <Link key={item.path} to={item.path} className={`portfolio-navbar-item ${isActiveItem(item) ? "active" : ""}`}>
                    {item.label}
      ...
      {showMobileNav && <nav className="mobile-bottom-nav md:hidden">
          {navItems.slice(0, 5).map(item => item.path === '/' ? <Link key={item.path} to={item.path} className={`mobile-bottom-nav-item ${isActiveItem(item) ? 'active' : ''}`} onClick={() => setIsOpen(false)}>
                <span className="mobile-bottom-nav-item-icon">
                  <item.icon size={20} />
                </span>
                <span className="mobile-bottom-nav-item-label">{item.label}</span>
              </Link> : <button key={item.path} onClick={() => handleNavClick(item)} className={`mobile-bottom-nav-item ${isActiveItem(item) ? 'active' : ''}`}>
                <span className="mobile-bottom-nav-item-icon">
                  <item.icon size={20} />
                </span>
                <span className="mobile-bottom-nav-item-label">{item.label}</span>
